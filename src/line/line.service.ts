...
export class LineService {
  ...
  private async handleImageMessage(event: any): Promise<void> {
    ...
    this.logger.log(
      `Processing image. msgId: ${event.message.id}, userId: ${userId}`,
    );
    ...
    try {
      ...
      const savedReceipt = await this.receiptService.createReceipt(
        userId,
        receiptData,
      );
      ...
      if (event.replyToken) {
        ...
        this.logger.log(
          `Flex message sent to userId: ${userId}, receiptId: ${receiptId}`,
        );
      }
    } catch (error: any) {
      this.logger.error(
  ...
  private async handlePostback(event: any): Promise<void> {
    ...
    const liffEditUrl =
      process.env.LIFF_EDIT_URL ?? 'https://liff.line.me/your-liff-id';
    ...
    this.logger.log(
      `[POSTBACK] Full edit URL would be: "${liffEditUrl}?id=${receiptId}"`,
    );
    ...
    if (action === 'approve') {
      try {
        ...
        const flexMessage = buildFinalReceiptFlexMessage(
          receiptData,
          doc.status ?? 'approved',
        );
        ...
        this.logger.error(
          `Failed to approve receipt ${receiptId}: ${err.message}`,
        );
        await this.safeReplyText(
          event.replyToken,
          'ขออภัย ไม่สามารถอัปเดตสถานะได้',
        );
      }
      ...
      try {
        ...
        this.logger.error(
          `Failed to cancel receipt ${receiptId}: ${err.message}`,
        );
        await this.safeReplyText(
          event.replyToken,
          'ขออภัย ไม่สามารถยกเลิกใบเสร็จได้',
        );
      }
    } else {
  ...
  async sendFinalReceiptMessage(
    ...
    try {
      ...
      this.logger.log(
        `Final flex message sent to userId: ${userId} with status: ${status}`,
      );
    } catch (error: any) {
      this.logger.error(
        `Failed to push final receipt message to ${userId}: ${error.message}`,
      );
    }
  }
}
