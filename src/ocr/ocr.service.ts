...
export class OcrService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (!apiKey) {
      ...
      this.logger.warn(
        'GEMINI_API_KEY is not set. Receipt parsing will fail until it is configured in .env',
      );
...
${rawText}`;
    ...
    this.logger.log(
      `Parsed receipt: ${data.storeName}, ฿${data.totalAmount}, ${data.items.length} items`,
    );
    return data;
  }
  ...
  async processReceipt(imageBuffer: Buffer): Promise<ReceiptData> {
    ...
    if (!rawText || rawText.trim().length === 0) {
      this.logger.warn(
        'Vision API returned empty text. Returning empty receipt data.',
      );
      return {
