...
export class ReceiptController {
  ...
  async getReceipt(@Param('id') id: string): Promise<Receipt> {
    ...
    try {
      ...
      this.logger.log(
        `[GET /api/receipts/${id}] found → storeName="${doc.storeName}", status="${doc.status}"`,
      );
      return doc;
    } catch (err: any) {
  ...
  async updateReceipt(
    ...
    this.logger.log(
      `[PUT /api/receipts/${id}] body keys: ${Object.keys(body).join(', ')}`,
    );
    try {
      ...
      if (body.status === 'approved' || body.status === 'cancelled') {
        ...
        await this.lineService.sendFinalReceiptMessage(
          doc.userId,
          receiptData,
          doc.status,
        );
